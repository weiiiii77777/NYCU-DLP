...
class iclevrDataset(Dataset):
    def __init__(self, root='../dataset/iclevr', _transforms=None, mode="train",):
        super().__init__()
        assert mode in ['train', 'test', 'new_test', 'denoise'], "There is no such mode !!!"
        ...
        if mode == 'train':
            ...
            with open('../dataset/new_test.json', 'r') as json_file:
                self.json_data = json.load(json_file)
            self.labels = self.json_data
        elif mode == 'denoise':
            with open('../dataset/denoise.json', 'r') as json_file:
                self.json_data = json.load(json_file)
            self.labels = self.json_data
        # initailize one-hot labels
        self.labels_one_hot = []
        with open('../dataset/objects.json', 'r') as json_file:
