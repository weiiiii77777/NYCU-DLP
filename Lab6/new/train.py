...
def train(G, D, opt_G, opt_D, adversarial_loss, auxiliary_loss, train_loader, test_loader, args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        save_image(gen_images, os.path.join(f'{args.path}/pic/', f'epoch{epoch}.png'), nrow=8)
        if eval_acc > best_acc or eval_acc > 70:
            if eval_acc > best_acc :
                best_acc = eval_acc
            torch.save(G.state_dict(), os.path.join(f'{args.path}/', f'epoch{epoch}_acc{eval_acc:.3f}.pth'))
    
    # plot(eval_acc_list, avg_loss_G_list, avg_loss_D_list, args)
    f = open(os.path.join(args.path, f"record.txt"), 'w')
    print('D Loss', file=f)
    for i in range(args.epoch):
        print(round(avg_loss_D_list[i], 5),',', end=' ', file=f)
        if i%10 == 9:
            print(file=f)
    print(file=f)
    print('G Loss', file=f)
    for i in range(args.epoch):
        print(round(avg_loss_G_list[i], 5),',', end=' ', file=f)
        if i%10 == 9:
            print(file=f)
    print(file=f)
    print('Accuracy', file=f) 
    for i in range(args.epoch):
        print(round(eval_acc_list[i], 5),',', end=' ', file=f)
        if i%10 == 9:
            print(file=f)
    print(file=f)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default='cuda:1', help='Size of z latent vector')
    ...
    parser.add_argument('--path', type=str, default='./save_model')
