...
# python .\faster-pytorch-fid\fid_score_gpu.py --predicted-path "C:\Users\percy\DL\lab5\test_results" --gtcsv-path "C:\Users\percy\DL\lab5\faster-pytorch-fid\test_gt.csv" --device cuda:0 --num-workers 0
...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i,iter,path): #MakGIT inference
        # image: 1, 3, 64, 64
        # mask_b: 1, 256
        ...
        self.model.eval()
        with torch.no_grad():
            _, z_indices = self.model.encode_to_z(image.to(device=self.device)) #z_indices: masked tokens (b,16*16)
            ...
            for step in range(iter):
                if step == self.sweet_spot:
                    break
                ratio = self.model.gamma(step/iter) #this should be updated
                # print(step/self.total_iter, ratio)
            ...
            vutils.save_image(dec_img_ori, os.path.join("result/"+path+"/pic", f"image_{i:03d}.png"), nrow=1) 

            #demo score 
            vutils.save_image(maska, os.path.join("result/"+path+"/mask", f"test_{i}.png"), nrow=10) 
            vutils.save_image(imga, os.path.join("result/"+path+"/imga", f"test_{i}.png"), nrow=7)
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/transformer_epoch_10.pth', help='load ckpt')
    ...
    print(MaskGit_CONFIGS)
    for iter in range(5, 31) :
        args.total_iter = iter
        path = '7_90_diff_T/square/' + str(iter)
        print(iter)
        if not os.path.isdir('./result/' + path + '/imga') : 
            os.makedirs('./result/' + path + '/imga')
        if not os.path.isdir('./result/' + path + '/mask') :
            os.makedirs('./result/' + path + '/mask')
        if not os.path.isdir('./result/' + path + '/pic') :
            os.makedirs('./result/' + path + '/pic')
        i=0
        for image, mask in zip(t.mi_ori, t.mask_ori):
            ...
            maskgit.inpainting(image,mask_b,i,iter,path)
            i+=1